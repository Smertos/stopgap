...
- Shared helper migration has started via `crates/common` (SQL quoting + bool-setting parsing); stopgap pure domain/state-transition logic now also lives in `crates/stopgap/src/domain.rs` as an incremental `lib.rs` split step, while broader module split and test-file granularity extraction remain pending.
