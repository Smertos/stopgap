...
- Shared helper migration has started via `crates/common` (SQL quoting + bool-setting parsing); plts compile/fingerprint/source-map helpers now live in `crates/plts/src/compiler.rs`, plts runtime SPI/query-binding helpers now live in `crates/plts/src/runtime_spi.rs`, plts function source/artifact-pointer cache helpers now live in `crates/plts/src/function_program.rs`, plts regular-arg payload conversion helpers now live in `crates/plts/src/arg_mapping.rs`, stopgap pure domain/state-transition logic lives in `crates/stopgap/src/domain.rs`, runtime config/SPI helpers live in `crates/stopgap/src/runtime_config.rs`, deploy scan/materialization helpers live in `crates/stopgap/src/deployment_utils.rs`, stopgap deployment-state/rollback helpers live in `crates/stopgap/src/deployment_state.rs`, stopgap deploy/status/diff orchestration helpers live in `crates/stopgap/src/api_ops.rs`, stopgap role/permission helpers live in `crates/stopgap/src/security.rs`, stopgap SQL API/bootstrap wiring now lives in `crates/stopgap/src/api.rs` + `crates/stopgap/src/sql_bootstrap.rs` with a thin `crates/stopgap/src/lib.rs`, focused `pg_regress` scenario files now cover deploy/rollback/prune/diff/security, and broader `plts` module split remains pending.
