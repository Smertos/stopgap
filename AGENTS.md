...
- `plts.compile_ts` now transpiles TS->JS via `deno_ast`, reports structured diagnostics, and records compiler fingerprint metadata from lockfile-resolved dependency versions; artifact source-map persistence is still pending.
