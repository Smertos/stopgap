...
- Shared helper migration has started via `crates/common` (SQL quoting + bool-setting parsing); plts compile/fingerprint/source-map helpers now live in `crates/plts/src/compiler.rs`, stopgap pure domain/state-transition logic lives in `crates/stopgap/src/domain.rs`, runtime config/SPI helpers live in `crates/stopgap/src/runtime_config.rs`, deploy scan/materialization helpers live in `crates/stopgap/src/deployment_utils.rs`, and broader module split plus focused `pg_regress` scenario expansion remain pending.
