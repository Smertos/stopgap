...
#[pg_test]
fn test_artifact_pointer_supports_named_export_entrypoint() {
    Spi::run(
        "
        DROP SCHEMA IF EXISTS plts_runtime_ptr_named_it CASCADE;
        CREATE SCHEMA plts_runtime_ptr_named_it;
        ",
    )
    ...
    let create_sql = format!(
        "
        CREATE OR REPLACE FUNCTION plts_runtime_ptr_named_it.ptr_fn(args jsonb)
        RETURNS jsonb
        LANGUAGE plts
        AS $$ {} $$;
        ",
    ...
    let payload = Spi::get_one::<JsonB>(
        "SELECT plts_runtime_ptr_named_it.ptr_fn('{\"id\": 7, \"tag\": \"ok\"}'::jsonb)",
    )
    .expect("named-export pointer function invocation should succeed")
    .expect("named-export pointer function should return jsonb");

    assert_eq!(payload.0.get("route").and_then(Value::as_str), Some("named"));
    assert_eq!(
        payload.0.get("echoed").and_then(|value| value.get("id")).and_then(Value::as_i64),
        Some(7)
    );

    Spi::run("DROP SCHEMA IF EXISTS plts_runtime_ptr_named_it CASCADE;")
        .expect("named-export artifact-pointer teardown SQL should succeed");
}
