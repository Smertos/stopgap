...
- [x] Add stopgap deploy plumbing to emit/manage import maps for live pointer functions so runtime bare-specifier mapping does not require manual inline comments in function sources.
...
- [x] Remove coarse-clock zero-elapsed flakes from measurable-time assertions.
...
- [x] `plts` now requires `shared_preload_libraries = 'plts'` for compile/typecheck SQL APIs and routes those requests through a single shared compiler worker with DSM + `shm_mq` transport, a persistent TSGo Wasm reactor, and new `plts.compile_ts_checked(...)` coverage (`crates/plts/src/compiler_service.rs`, `crates/plts/src/api.rs`, `third_party/stopgap-tsgo-api/api/reactor.go`).
